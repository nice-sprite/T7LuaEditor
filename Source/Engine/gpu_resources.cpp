...
HRESULT create_constant_buffer(ID3D11Device* device, size_t bufferSizeInBytes, ID3D11Buffer** ppBuffer)
{
...
HRESULT create_constant_buffer(ID3D11Device* device, void* data, size_t bufferSize, ID3D11Buffer** ppBuffer)
{
    D3D11_BUFFER_DESC bd{};
    bd.Usage = D3D11_USAGE_DYNAMIC;
...
HRESULT create_dynamic_vertex_buffer(ID3D11Device* device, ID3D11Buffer** ppBuffer, void* initialData, int sizeInBytes) 
{
    D3D11_BUFFER_DESC bd{};
...
void bind_dynamic_vertex_buffers(ID3D11DeviceContext* context, ID3D11Buffer** buf, UINT stride, UINT offset)
{
    context->IASetVertexBuffers(0, 1, buf, &stride, &offset);
}

// updates the entire vertex buffer, discards previous contents
void update_dynamic_vertex_buffer(ID3D11DeviceContext* context, ID3D11Buffer* buf, void* vertexData, size_t sizeInBytes)
{
    D3D11_MAPPED_SUBRESOURCE msr{};
    context->Map(buf, 0, D3D11_MAP_WRITE_DISCARD, 0, &msr);
