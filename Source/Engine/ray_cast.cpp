...
namespace ray_cast {
    ...
    bool point_inside_rect(XMVECTOR point, XMFLOAT4 rect) {
            return XMVectorGetX(point) > rect.x && 
            XMVectorGetX(point) < rect.y && 
            XMVectorGetY(point) > rect.z && 
            XMVectorGetY(point) < rect.w;
    }
    ...
    bool volume_intersection(Ray mins, Ray maxs, XMFLOAT4 quad) {
        ...
        // check for nan
        bool not_nan = !XMVectorGetIntX(XMVectorIsNaN(mins_intersection)) && !XMVectorGetIntX(XMVectorIsNaN(maxs_intersection));
