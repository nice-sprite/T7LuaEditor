...
void DebugRenderSystem::init(Renderer &renderer) {
  ...
  renderer.create_vertex_shader(debug_line_shader_path,
                                VertexPosColor::layout(), &line_vertex_shader,
                                &vertex_layout);
...
void DebugRenderSystem::debug_line_vec4(XMVECTOR a, XMVECTOR b,
                                        XMFLOAT4 color) {
...
void DebugRenderSystem::update_line_vec4(u32 i, XMVECTOR a, XMVECTOR b,
                                         XMFLOAT4 color) {
...
void DebugRenderSystem::update_line_float3(u32 i, XMFLOAT3 a, XMFLOAT3 b,
                                           XMFLOAT4 color) {
  if (i >= 0 && i < MaxDebugLines) {
    lines[i].begin = a;
    lines[i].end = b;
...
void DebugRenderSystem::update(Renderer &renderer) {
  renderer.update_buffer(
      vertex_buffer.Get(), [=](D3D11_MAPPED_SUBRESOURCE &msr) {
...
void DebugRenderSystem::draw(Renderer &renderer) {
  ...
  renderer.set_vertex_buffer(vertex_buffer.GetAddressOf(), 1,
                             sizeof(VertexPosColor), 0);
