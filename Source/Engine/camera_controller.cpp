...
void flycam_fps(float dt, Camera &camera) {
  ...
  if (input.mouse_buttons.left) {
    ...
    camera.pitch = clamp_float(camera.pitch,
  ...
  camera.update_view(XMMatrixLookAtLH(camera.origin,
                                      camera.origin + camera_target,
                                      camera.up));
...
void dollycam(float dt, Camera &camera) {
  ...
  if (input.key_down(VK_LCONTROL)) {
    camera_offset +=
        XMVectorSet(0.0,
                    0.0,
                    movement_speed * dt * (float)(input.mouse_wheel),
                    0.0);
    if (input.mouse_buttons.left) {
      camera_offset += XMVectorSet(-(float)(input.mouse_delta.dx),
                                   (float)(input.mouse_delta.dy),
                                   0.0,
                                   0.0);
    }
  }
