...
inline float clamp_float(float a, float low, float high) {
  return min_float(high, max_float(a, low));
}
...
void flycam_fps(float dt, Camera &camera) {
  ...
  if (Input::GameInput::mouse_button_down(GameInputMouseRightButton)) {
    ...
    camera.pitch = clamp_float(camera.pitch, (float)(-XM_PIDIV2 + 0.001),
                               (float)(XM_PIDIV2 - 0.001));

    if (Input::GameInput::key_down('W')) {
      camera_offset =
          XMVectorAdd(camera_offset, XMVectorSet(0.0, 0.0, 1.0, 0.0));
    }
    ...
    if (Input::GameInput::key_down('A')) {
      camera_offset =
          XMVectorAdd(camera_offset, XMVectorSet(-1.0, 0.0, 0.0, 0.0));
    }
  ...
  rotation =
      XMMatrixRotationRollPitchYaw(camera.pitch, camera.yaw, camera.roll);
  temp_y_matrix = XMMatrixRotationY(camera.yaw);

  if (Input::GameInput::key_down(VK_LSHIFT)) {
    movement_speed = 1500.0f;
  } else if (Input::GameInput::key_down(VK_LCONTROL)) {
    movement_speed = 100.0;
  }

  camera.origin +=
      dt * movement_speed *
      XMVector3Normalize(XMVector3TransformCoord(camera_offset, rotation));

  if (Input::GameInput::key_down('E'))
    camera.origin += dt * movement_speed * XMVectorSet(0.0, 1.0, 0.0, 0.0);
  if (Input::GameInput::key_down('Q'))
    camera.origin += dt * movement_speed * XMVectorSet(0.0, -1.0, 0.0, 0.0);

  camera_target =
      XMVector3Normalize(XMVector3TransformCoord(default_forward, rotation));
  ...
  camera.update_view(XMMatrixLookAtLH(
      camera.origin, camera.origin + camera_target, camera.up));

#ifdef DEBUG_CAMERA
  if (ImGui::Begin("flycam fps debug")) {
    ImGui::Text("time_delta: %f", dt);
    ImGui::Text("mouse delta (%d, %d)", Input::GameInput::mouse_delta().dx,
                Input::GameInput::mouse_delta().dy);

    ImGui::Separator();
    ImGui::Text("origin: %.2f %.2f %.2f", XMVectorGetX(camera.origin),
                XMVectorGetY(camera.origin), XMVectorGetZ(camera.origin));
    ImGui::Text("pitch: %f | yaw: %f", camera.pitch, camera.yaw);
    ImGui::Text("target: %.2f %.2f %.2f", XMVectorGetX(camera_target),
...
void dollycam(float dt, Camera &camera) {
  ...
  if (Input::GameInput::key_down(VK_LCONTROL)) {
    camera_offset += XMVectorSet(
        0.0, 0.0,
        movement_speed * dt * (float)(Input::GameInput::mouse_delta().wheel_y),
        0.0);
    if (Input::GameInput::mouse_button_down(GameInputMouseLeftButton)) {
      camera_offset +=
          XMVectorSet(-(float)(Input::GameInput::mouse_delta().dx),
                      (float)(Input::GameInput::mouse_delta().dy), 0.0, 0.0);
    }
  }
  ...
  if (ImGui::Begin("dollycam debug")) {
    ...
    ImGui::Text("scroll: %d %d", Input::GameInput::mouse_delta().wheel_x,
